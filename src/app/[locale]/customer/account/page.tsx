...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.push(withLocale(`${locale}/customer/login`));
      return;
    }
  ...
  const fetchOrders = async (token: string) => {
    try {
      const res = await apiClient.get(`${Endpoints.order}/my-orders`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const cities =
    user.address.governorate
      ? egyptData[user.address.governorate as keyof typeof egyptData] || []
      : [];
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-6 md:p-10">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row-reverse items-center md:justify-between gap-6 mb-8">
          ...
          >
            {edit ? t("shipping.save") : t("dataTable.edit")}
          </Button>
            ...
            <CardContent className="grid md:grid-cols-2 gap-4">
              ...
              {edit ? (
                <Dropdown
                  ...
                  options={Object.keys(egyptData).map((gov) => ({
                    label: gov,
                    value: gov,
                  }))}
                  onChange={(e) =>
